...
class DailyReport:
    def __init__(self):
        self.yesterday = None  # should populated while report running

    def run_and_record_for_yesterday(self, *, verify_yesterday=None, also_send_to_keen=False):
        time_report_started = timezone.now()
        self.yesterday = (time_report_started - timedelta(days=1)).date()

        if verify_yesterday is not None:
            agreed_on_yesterday = (verify_yesterday == self.yesterday)
            if not agreed_on_yesterday:
                raise WrongYesterday(verify_yesterday, my_yesterday=self.yesterday)
        ...
        if also_send_to_keen:
            ...
            keen_events = self.get_keen_events(daily_report, keen_event_timestamp)
    ...
    def backfill_report(self, date):
        raise NotImplementedError(
            f'{self.__name__} must implement backfill_report'
            ' (can simply raise `mourningwail.exceptions.CannotBackfillReport`)'
        )

    def get_keen_events(self, daily_report, keen_event_timestamp):
